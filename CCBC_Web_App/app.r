...
#summaryRprof(filename = "Rprof.out", memory = "stats")


# Source Functions --------------------------------------------------------

source(here("scripts/html_functions.R"))
...
advanced_hitting_tbl  <-  readRDS(here("data/advanced_hitting_tbl.rds"))
standard_hitting_tbl  <-  readRDS(here("data/standard_hitting_tbl.rds"))
base_running_tbl      <-  readRDS(here("data/base_running_tbl.rds"))
standard_pitching_tbl <-  readRDS(here("data/standard_pitching_tbl.rds"))
team_stats_tbl        <-  readRDS(here("data/team_stats_final_tbl.rds"))
...
ui <- function(req) {
    if (identical(req$REQUEST_METHOD, "GET")) {
        ...
        #Handle the post
        #add in authentication later
        print("test")
        content_text <- '{"status": "ok"}' 
        status_code <- 200L
        query_params <- parseQueryString(req$QUERY_STRING)
        body_bytes <- req$rook.input$read(-1)
        body_rds <- unserialize(body_bytes)
        
        print(query_params$dataset)
        if (identical(query_params$dataset, "S_HITTING")) {
            standard_hitting_tbl  <-  body_rds        
        } else if (identical(query_params$dataset, "A_HITTING")) {
            advanced_hitting_tbl  <-  body_rds    
        } else if (identical(query_params$dataset, "BASERUNNING")) {
            base_running_tbl  <- body_rds 
        } else if (identical(query_params$dataset, "S_PITCHING")) {
            standard_pitching_tbl  <-  body_rds    
        } else if (identical(query_params$dataset, "TEAM")) {
            team_stats_tbl  <-  body_rds    
        } else {
            content_text  <-  '{"status": "Failed - invalid dataset"}'
            status_code  <-  500
        }
...
#shiny_env$resourcePaths <- c(shiny_env$resourcePaths, list("data" = httpuv::excludeStaticPath()))
